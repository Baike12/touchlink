...
const updateChart = () => {
  ...
  // 添加调试信息
  console.log('柱状图数据:', chartData);
  console.log('柱状图配置:', chartOptions);
  
  // 如果没有数据，显示空图表
  if (!chartData || chartData.length === 0) {
    chartInstance.setOption({
      title: {
        text: typeof chartOptions.title === 'string' ? chartOptions.title : (chartOptions.title?.text || '柱状图'),
        left: 'center'
      },
      tooltip: {
        formatter: '暂无数据'
