...
export function getDataSourceTypes() {
  return get<string[]>('/api/v1/datasources/types')
}

// 测试数据源连接
export function testDataSourceConnection(config: DataSourceConfig) {
  return post<DataSourceResponse>('/api/v1/datasources/test', config)
}

// 连接数据源
export function connectDataSource(config: DataSourceConfig) {
  return post<DataSourceResponse>('/api/v1/datasources/connect', config)
}

// 保存数据源
export function saveDataSource(request: DataSourceCreateRequest) {
  return post<DataSourceDetail>('/api/v1/datasources', request)
}
...
export function getTables(datasourceId?: string) {
  ...
  const url = datasourceId 
    ...
    : `/api/v1/datasources/session/tables`;
...
export function getTableSchema(tableName: string, datasourceId?: string) {
  // 如果没有数据源ID，则使用当前会话中的临时连接
  const url = datasourceId
    ? `/api/v1/datasources/${datasourceId}/tables/${tableName}/schema`
    : `/api/v1/datasources/tables/${tableName}/schema`;
...
export function getTableData(tableName: string, limit: number = 100, datasourceId?: string) {
  // 如果没有数据源ID，则使用当前会话中的临时连接
  const url = datasourceId
    ? `/api/v1/datasources/${datasourceId}/tables/${tableName}/data`
    : `/api/v1/datasources/tables/${tableName}/data`;
  return get<TableDataResponse>(`${url}?limit=${limit}`);
}

// 获取所有数据源
export function getDataSources() {
  return get<DataSourceDetail[]>('/api/v1/datasources')
}

// 获取数据源详情
export function getDataSourceDetail(id: string) {
  return get<DataSourceDetail>(`/api/v1/datasources/${id}`)
}

// 删除数据源
export function deleteDataSource(id: string) {
  return del(`/api/v1/datasources/${id}`)
} 