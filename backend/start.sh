...
start_app() {
    ...
    # 启动应用
    echo -e "${GREEN}执行命令: $PYTHON_CMD -m uvicorn src.main:app --reload --host 0.0.0.0 --port 8000${NC}"
    $PYTHON_CMD -m uvicorn src.main:app --reload --host 0.0.0.0 --port 8000 > "$LOG_FILE" 2>&1 &
